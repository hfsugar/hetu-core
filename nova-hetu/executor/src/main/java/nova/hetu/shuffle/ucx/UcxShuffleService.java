        ...
        implements Runnable, Closeable
{
    ...
    private void processRequest(ByteBuffer buffer)
    {
        ...
        switch (msgType) {
            ...
            case TAKE:
                ...
                log.info("Server handle TAKE [" + take.getId() + "] message.");
                stream.enqueue(take.getRateLimit());
                break;
            case CLOSE:
                // close stream
                UcxCloseMessage close = new UcxCloseMessage(buffer);
                log.info("Server handle CLOSE [" + close.getId() + "] [" + close.getProducerId() + "]message.");
            ...
            implements Closeable
    {
        ...
        public UcxStream(String producerId, int id)
        {
            ...
            this.id = id;
            this.stream = StreamManager.get(producerId);
            this.sendId = 0;
        }
        ...
        private RegisteredMemory onHeapMemory(long tag, SerializedPage page)
        {
            ...
            log.info("Server send on heap page: [" + tag + "]" + builder.toString());
        ...
        private RegisteredMemory offHeapMemory(long tag, SerializedPage page)
        {
            ...
            log.info("Server send off heap page: [" + tag + "]" + builder.toString());
